...
<fieldset class="layui-elem-field">
	...
	<div class="layui-field-box">
			...
			<div class="layui-form-item" style="text-align:center;">
				<div style="display:inline;width:300px">
					...
					<div class="layui-inline" style="margin-top:20px;">
						<div>
							<input type="text"  style="display:inline;width:200px" value="" placeholder="请输入评论内容" id="content" class="layui-input">
						</div>
					</div>
				</div>
		...
		<hr>
			...
			<hr>
				<table class="layui-table"   style="width:100%; table-layout:inherit; word-break: break-all;overflow: auto;">
					...
					<tbody id="news_dis_body">
					</tbody>
				</table>
				<div id="news_dis_bar" class="pagination" style="margin-top:20px;"></div>
			</div>
		</fieldset>
		...
		<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=3795"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/news_dis/queryList",
		data : {
			"page":page,
			"content":content
		},
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				html+='<td><input name="itemCheckBox" data-id="' + rows[i].newsDis.id+ '" type="checkbox"></td>';
				var contentVal=setNullToEmpty(rows[i].newsDis.content);
				html+='<td>'+ contentVal+'</td>';
				var nidVal=setNullToEmpty(rows[i].nidStr);
				html+='<td>'+ nidVal+'</td>';
				var uidVal=setNullToEmpty(rows[i].uidStr);
				html+='<td>'+ uidVal+'</td>';
				var createTimeVal=setNullToEmpty(rows[i].newsDis.createTime);
				html+='<td>'+ createTimeVal+'</td>';
				html+='<td>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].newsDis.id+'" class="layui-btn layui-btn-danger layui-btn-radius btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/news_dis/queryCount",
		async: false,
		data : {
			"content":content
		},
...
function batchAction(){
	...
	if(r==true){
		var url="${pageContext.request.contextPath}/admin/news_dis/"+type+"";
		$.ajax({
			type : 'get',
			url : url,
			data : {
...
function del(e){
	if(window.confirm('你确定要删除该新闻评论吗')){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/news_dis/del",
			data : {
				"id":id
			},
			success : function(result) {
