...
<fieldset class="layui-elem-field">
	...
	<div class="layui-field-box">
		...
		<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=1674"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/join_log/queryList",
		data : {
			"aid":"${data.aid}",
			"page":page,
			"urealName":urealName
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				html+='<td><input name="itemCheckBox" data-id="' + rows[i].joinLog.id+ '" type="checkbox"></td>';
				...
				html+='<td>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].joinLog.id+'" class="layui-btn layui-btn-danger layui-btn-radius btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/join_log/queryCount",
		async: false,
		data : {
			"aid":"${data.aid}",
			"urealName":urealName
...
function batchAction(){
	...
	if(r==true){
		var url="${pageContext.request.contextPath}/admin/join_log/"+type+"";
		$.ajax({
			type : 'get',
			url : url,
			data : {
...
function del(e){
	if(window.confirm('你确定要删除该参加记录吗')){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/join_log/del",
			data : {
				"id":id
			},
			success : function(result) {
