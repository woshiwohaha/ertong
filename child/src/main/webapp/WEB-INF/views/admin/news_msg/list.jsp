...
<fieldset class="layui-elem-field">
	...
	<div class="layui-field-box">
			...
			<div class="layui-form-item" style="text-align:center;">
				<div style="display:inline;width:300px">
					...
					<div class="layui-inline" style="margin-top:20px;">
						<div>
							<input type="text"  style="display:inline;width:200px" value="" placeholder="请输入标题" id="newsName" class="layui-input">
						</div>
					</div>
				</div>
		...
		<hr>
			<div class="layui-form-item">
				...
				<span class="l" id="globalBtns">
					<a href="${pageContext.request.contextPath}/admin/news_msg/add" class="layui-btn layui-btn-radius">新增</a>
				</span>
			</div>
			<hr>
				<table class="layui-table"   style="width:100%; table-layout:inherit; word-break: break-all;overflow: auto;">
					...
					<tbody id="news_msg_body">
					</tbody>
				</table>
				<div id="news_msg_bar" class="pagination" style="margin-top:20px;"></div>
			</div>
		</fieldset>
		...
		<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=3402"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/news_msg/queryList",
		data : {
			"page":page,
			"newsName":newsName
		},
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				html+='<td><input name="itemCheckBox" data-id="' + rows[i].newsMsg.id+ '" type="checkbox"></td>';
				var newsNameVal=setNullToEmpty(rows[i].newsMsg.newsName);
				html+='<td>'+ newsNameVal+'</td>';
				var noticeImgStr = rows[i].newsMsg.noticeImg;
				if(noticeImgStr!=null){
					var noticeImgSplit = noticeImgStr.split(";");
					var tmp = '';
					for(var j=0;j<noticeImgSplit.length;j++){
						if(noticeImgSplit[j]!=''){
							tmp+='<div style="display:inline"><img src="'+noticeImgSplit[j]+'" style="width:100px;height:100px" onclick="maxImg(this)"></div>';
						}
					}
					html+='<td>'+tmp+'</td>';
					}else{
				...
				html+='<td>'+ aidVal+'</td>';
				var createTimeVal=setNullToEmpty(rows[i].newsMsg.createTime);
				html+='<td>'+ createTimeVal+'</td>';
				html+='<td>';
				html+='<a href="${pageContext.request.contextPath}/admin/news_msg/detail1?id='+rows[i].newsMsg.id+'" class="layui-btn layui-btn-radius btnStyle">查看详情</a>';
				html+='<a href="${pageContext.request.contextPath}/admin/news_msg/update?id='+rows[i].newsMsg.id+'" class="layui-btn layui-btn-normal layui-btn-radius btnStyle">修改</a>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].newsMsg.id+'" class="layui-btn layui-btn-danger layui-btn-radius btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/news_msg/queryCount",
		async: false,
		data : {
			"newsName":newsName
		},
...
function batchAction(){
	...
	if(r==true){
		var url="${pageContext.request.contextPath}/admin/news_msg/"+type+"";
		$.ajax({
			type : 'get',
			url : url,
			data : {
...
function del(e){
	if(window.confirm('你确定要删除该新闻吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/news_msg/del",
			data : {
				"id":id
			},
			success : function(result) {
