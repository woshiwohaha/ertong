...
<fieldset class="layui-elem-field">
	...
	<div class="layui-field-box">
			...
			<div class="layui-form-item" style="text-align:center;">
				<div style="display:inline;width:300px">
					...
					<div class="layui-inline" style="margin-top:20px;">
						<div>
							<input type="text"  style="display:inline;width:200px" value="" placeholder="请输入登录名" id="loginName" class="layui-input">
						</div>
					</div>
				</div>
				<div style="display:inline;width:300px">
					...
					<div class="layui-inline" style="margin-top:20px;">
						<div>
							<input type="text"  style="display:inline;width:200px" value="" placeholder="请输入昵称" id="realName" class="layui-input">
						</div>
					</div>
				</div>
		...
		<hr>
			<div class="layui-form-item">
				...
				<span class="l" id="globalBtns">
					<a href="${pageContext.request.contextPath}/admin/user_msg/add" class="layui-btn layui-btn-radius">新增</a>
				</span>
				<span class="l" id="export">
					<a href="${pageContext.request.contextPath}/admin/user_msg/export" class="layui-btn layui-btn-radius">一键导出</a>
				</span>
				<span class="l" id="import">

        <button id="upload" class="layui-btn layui-btn-radius">一键导入</button>
        <input type="file" id="fileCont" >
		...
		<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=9429"></script>
	...
	$.ajax({
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				html+='<td><input name="itemCheckBox" data-id="' + rows[i].userMsg.id+ '" type="checkbox"></td>';
				...
				var addressVal=setNullToEmpty(rows[i].userMsg.address);
				html+='<td>'+ addressVal+'</td>';
				var createTimeVal=setNullToEmpty(rows[i].userMsg.createTime);
				html+='<td>'+ createTimeVal+'</td>';
				html+='<td>';
				html+='<a href="${pageContext.request.contextPath}/admin/user_msg/update?id='+rows[i].userMsg.id+'" class="layui-btn layui-btn-normal layui-btn-radius btnStyle">修改</a>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].userMsg.id+'" class="layui-btn layui-btn-danger layui-btn-radius btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function batchAction(){
	...
	if(r==true){
		var url="${pageContext.request.contextPath}/admin/user_msg/"+type+"";
		$.ajax({
			type : 'get',
			url : url,
			data : {
...
// 一键导入
$("#fileCont").change(function(e){
	$("#i1").val($("#fileCont").val());
	var fileObj = $("#fileCont")[0].files[0];
	...
	var data = formFile;
	$.ajax({
		url:'${pageContext.request.contextPath}/admin/user_msg/import',
		data: data,
		type: "POST",
		dataType: "json",
		//上传文件无需缓存
