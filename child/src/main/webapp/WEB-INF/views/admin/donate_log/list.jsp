...
<fieldset class="layui-elem-field">
	...
	<div class="layui-field-box">
		...
		<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=5012"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/donate_log/queryList",
		data : {
			"hid":"${data.hid}",
			"page":page,
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				html+='<td><input name="itemCheckBox" data-id="' + rows[i].donateLog.id+ '" type="checkbox"></td>';
				...
				html+='<td>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].donateLog.id+'" class="layui-btn layui-btn-danger layui-btn-radius btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/donate_log/queryCount",
		async: false,
		data : {
			"hid":"${data.hid}",
			"contactMsg":contactMsg
...
function batchAction(){
	...
	if(r==true){
		var url="${pageContext.request.contextPath}/admin/donate_log/"+type+"";
		$.ajax({
			type : 'get',
			url : url,
			data : {
...
function del(e){
	if(window.confirm('你确定要删除该捐助记录吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/donate_log/del",
			data : {
				"id":id
			},
			success : function(result) {
