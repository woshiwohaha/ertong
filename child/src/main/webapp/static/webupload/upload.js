...
function addAImg(url,uploadType,domId,projectUri){
	...
	var html='<div style="margin-top:20px;">'
	 ...
	 html += '<img width="40px;" src="'+projectUri+'/static/webupload/close.png">';
	 html += '</a>';
		if(uploadType==1){
...
function createUpload(domId,uploadType,uploadNum,projectUri){
	...
	var uploader = WebUploader.create({
		swf : projectUri+'/static/webupload/Uploader.swf',
		server : projectUri+'/imgUpload/thumb',
		pick : '#'+domId+'Picker',
		resize : false,
		 duplicate :true
	});
	uploader.on('fileQueued', function(file) {
		...
		+'<div id="uploadContent'+barNum+'" style="width:400px;">'
		+'<img src="'+projectUri+'/static/webupload/imgloading.gif" style="width:400px;">'
